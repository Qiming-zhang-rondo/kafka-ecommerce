...
public class KafkaController : ControllerBase
{
    ...
    private readonly ILogger<KafkaController> logger;
    ...
    public KafkaController(IHttpClientFactory httpClientFactory, ILogger<KafkaController> logger)
    {
    ...
    public async Task<ActionResult> RunExperiment([FromBody] ExperimentConfig config)
    {
        ...
        if (0 == Interlocked.Exchange(ref usingResource, 1))
        {
            ...
            connection.Open();
            KafkaExperimentManager experimentManager = KafkaExperimentManager.BuildKafkaExperimentManager(httpClientFactory, config, connection);
