...
namespace Kafka.Metric;

public sealed class KafkaMetricManager : MetricManager
{

    public KafkaMetricManager(ISellerService sellerService, ICustomerService customerService, IDeliveryService deliveryService) : base(sellerService, customerService, deliveryService)
    {
    }
    ...
    protected override List<Latency> CollectFromCustomer(DateTime finishTime)
    {
        ...
        while (Shared.ProductUpdateOutputs.Reader.TryRead(out TransactionOutput item))
        {
        ...
        while (Shared.CheckoutOutputs.Reader.TryRead(out TransactionOutput item))
        {
            ...
            {
                dupFin++;
                logger.LogDebug("[Customer] Duplicate finished CUSTOMER_SESSION transaction entry found: Tid {0} Mark {1}", item.tid, item);
            }
        }
